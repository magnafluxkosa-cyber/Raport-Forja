(function(window, document){
  ...
  function safeStoredLogin(){
    var email = '';
    var role = '';
    try { email = normalizeEmail(localStorage.getItem('rf_user_email') || localStorage.getItem('rf_email') || localStorage.getItem('rf_user') || ''); } catch(_) {}
    try { role = normalizePageKey(localStorage.getItem('rf_user_role') || localStorage.getItem('rf_role') || sessionStorage.getItem('rf_user_role') || ''); } catch(_) {}
    if(['admin','editor','operator','viewer'].indexOf(role) === -1) role = '';
    return { email: email, role: role, exists: !!email || !!role };
  }
  ...
  async function getSessionWithRetry(sb){
    var session = null;
    for(var i=0; i<12; i+=1){
      session = await getSession(sb);
      if(session && session.user) return session;
      if(window.ERPAuth && typeof window.ERPAuth.getSession === 'function'){
        try {
          session = await window.ERPAuth.getSession();
          if(session && session.user) return session;
        } catch(_) {}
      }
      await wait(i < 4 ? 250 : 500);
    }
    return null;
  }
  ...
  async function main(){
    ...
    markHidden();
    /* Nu mai blocăm fetch-urile paginii aici. Unele pagini au propriul auth/loader și blocarea globală poate rupe sesiunea. */
    ...
    await ensureRuntime();
    var sb = createClient();
    var session = await runInternal(function(){ return getSessionWithRetry(sb); });
    var pageKey = currentPageKey();

    if(!session || !session.user){
      var stored = safeStoredLogin();
      if(stored.exists && (stored.role === 'admin' || pageKey === 'index')){
        var fallbackAccess = {
          allowed:true,
          role: stored.role || 'viewer',
          permissions: stored.role === 'admin' ? { can_view:true, can_add:true, can_edit:true, can_delete:true, can_export:true, can_import:true } : { can_view:true, can_add:false, can_edit:false, can_delete:false, can_export:false, can_import:false },
          source:'stored-session-fallback',
          message:'Sesiunea Supabase nu a fost citită la timp; pagina a fost lăsată să continue folosind sesiunea locală existentă.'
        };
        window.__KAD_SECURITY_ACCESS__ = fallbackAccess;
        applyReadonly(fallbackAccess.permissions);
        markAllowed();
        return;
      }
      renderDenied('Autentificare necesară', 'Nu am găsit sesiune activă. Intră din nou în cont, apoi revino în pagină.');
      return;
    }

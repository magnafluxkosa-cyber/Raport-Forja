(function(window){
  ...
  var registry = {
      ...
      items: [
        {
          type:'group', key:'group-forja', label:'FORJĂ', panelId:'subForja', panelMode:'tabs', sections:[
            ...
            { key:'group-forja-inventar', buttonKey:'group-forja-inventar', label:'INVENTAR', sectionId:'forjaInventar', layout:'subgrid', items:[
              page('inventar-otel','INVENTAR OȚEL'), page('inventar-debitat','INVENTAR DEBITAT'), page('inventar-forjat','INVENTAR FORJAT'), page('stoc-ramas-teoretic','STOC RĂMAS TEORETIC'), page('piese-ramase-forjate','PIESE RĂMASE FORJATE')
            ]}
          ]
        },
        { type:'group', key:'group-prelucrari', label:'PRELUCRĂRI MECANICE', panelId:'subPrelucrari', panelMode:'flat', sections:[{ key:'group-prelucrari-links', label:'Pagini', sectionId:'prelucrariLinks', layout:'subgrid', items:[ page('plan-livrari','PLAN LIVRĂRI'), page('planificare-prelucrari','PLANIFICARE PRELUCRĂRI'), page('inventar-prelucrari','INVENTAR PRELUCRĂRI'), page('raport-pm','RAPORT PM'), page('raport-pm-operatori','RAPORT ZILNIC OPERATORI PM') ] }] },
        ...
        { type:'group', key:'group-calitate', label:'CALITATE', panelId:'subCalitate', panelMode:'flat', sections:[{ key:'group-calitate-links', label:'Pagini', sectionId:'calitateLinks', layout:'subgrid', items:[ page('magnaflux','MAGNAFLUX'), page('magnaflux-operatori','MAGNAFLUX OPERATORI'), page('magnaflux-calendar','MAGNAFLUX CALENDAR'), page('duritate','DURITATE'), page('capabilitate-proces-forjare','CAPABILITATE PROCES FORJARE'), page('capabilitate-proces-prelucrari','CAPABILITATE PROCES PRELUCRĂRI'), page('fisa-control-ctc-forja','FISA DE CONTROL CTC FORJA'), page('probleme-ctc-forja','PROBLEME CTC FORJA'), page('rebut','REBUT'), page('rebut-pm-helper','REBUT PM HELPER'), page('rebut-pm-operatori','REBUT PM OPERATORI'), page('analiza-rebut-calitate','ANALIZĂ REBUT CALITATE') ] }] },
        ...
        { type:'group', key:'group-mentenanta', label:'MENTENANTA', panelId:'subMentenanta', panelMode:'flat', sections:[{ key:'group-mentenanta-links', label:'Pagini', sectionId:'mentenantaLinks', layout:'subgrid', items:[ page('registru-mentenanta','Registru de mentenanta'), page('mentenanta-preventiva','Mentenanță preventivă'), page('inductoare','INDUCTOARE') ] }] },
        { type:'group', key:'group-manuale-instructiuni', label:'MANUALE / INSTRUCȚIUNI', panelId:'subManualeInstructiuni', panelMode:'flat', sections:[{ key:'group-manuale-instructiuni-links', label:'Manuale și instrucțiuni', sectionId:'manualeInstructiuniLinks', layout:'subgrid', items:[ page('manual-cm-315t','Manual CM 3.15 T'), page('manual-cm-25t','Manual CM 2.5 T'), page('instructiuni-forjare','INSTRUCȚIUNI FORJARE'), page('instructiuni-ambalare','INSTRUCȚIUNI AMBALARE') ] }] },
    ...
    helperAcl: {
      buttonGroups: [],
      pageGroups: [
        { key:'forja-pages', label:'Foi FORJĂ', items:[ ['raport-forja','Raport forja'],['numeralkod','NUMERALKOD'],['intrari-otel','INTRĂRI OȚEL'],['debitate','DEBITATE'],['operator-debitare','OPERATOR DEBITARE'],['forjate','FORJATE'],['eficienta','EFICIENȚĂ'],['program-utilaje','PROGRAM UTILAJE'],['stoc-initial-otel','STOC INIȚIAL OȚEL'],['inventar-otel','INVENTAR OȚEL'],['inventar-debitat','INVENTAR DEBITAT'],['inventar-forjat','INVENTAR FORJAT'],['stoc-ramas-teoretic','STOC RĂMAS TEORETIC'],['piese-ramase-forjate','PIESE RĂMASE FORJATE'],['operator-debitare-pin','OPERATOR DEBITARE PIN'] ] },
        { key:'prelucrari-pages', label:'Foi PRELUCRĂRI MECANICE', items:[ ['plan-livrari','PLAN LIVRĂRI'],['planificare-prelucrari','PLANIFICARE PRELUCRĂRI'],['inventar-prelucrari','INVENTAR PRELUCRĂRI'],['raport-pm','RAPORT PM'],['raport-pm-operatori','RAPORT ZILNIC OPERATORI PM'] ] },
        ...
        { key:'quality-pages', label:'Calitate / Rebut', items:[ ['magnaflux','MAGNAFLUX'],['magnaflux-operatori','MAGNAFLUX OPERATORI'],['duritate','DURITATE'],['capabilitate-proces-forjare','CAPABILITATE PROCES FORJARE'],['capabilitate-proces-prelucrari','CAPABILITATE PROCES PRELUCRĂRI'],['magnaflux-calendar','MAGNAFLUX CALENDAR'],['fisa-control-ctc-forja','FISA DE CONTROL CTC FORJA'],['probleme-ctc-forja','PROBLEME CTC FORJA'],['rebut','REBUT'],['rebut-pm-operatori','REBUT PM OPERATORI'],['analiza-rebut-calitate','ANALIZĂ REBUT CALITATE'],['rebut-pm','REBUT PM'],['rebut-pm-helper','REBUT PM HELPER'],['calendar-operatori','CALENDAR OPERATORI'] ] },
        ...
        { key:'resurse-umane-pages', label:'Resurse umane / Pontaje / Evaluări', items:[ ['pontaj-forja','PONTAJ FORJA'],['pontaj-sef-echipa','PONTAJ ȘEF ECHIPĂ'],['pontaj-mecanici','PONTAJ MECANICI'],['pontaj-ctc','PONTAJ CTC'],['pontaj-prelucrari-mecanice','PONTAJ PRELUCRĂRI MECANICE'],['raport-forja-operatori-ore','ORE OPERATORI FORJĂ'],['bonus-lunar','BONUS LUNAR'],['calendar-operatori','CALENDAR OPERATORI'],['fisa-evaluare-angajati','FIȘĂ EVALUARE ANGAJAȚI'] ] },
        ...
        { key:'mentenanta-pages', label:'Foi MENTENANȚĂ', items:[ ['registru-mentenanta','REGISTRU DE MENTENANȚĂ'], ['mentenanta-preventiva','MENTENANȚĂ PREVENTIVĂ'], ['inductoare','INDUCTOARE'] ] },
        { key:'manuale-instructiuni-pages', label:'Manuale / Instrucțiuni', items:[ ['manual-cm-315t','Manual CM 3.15 T'], ['manual-cm-25t','Manual CM 2.5 T'], ['instructiuni-forjare','INSTRUCȚIUNI FORJARE'], ['ax-526-9278','AX-526-9278'], ['instructiuni-ambalare','INSTRUCȚIUNI AMBALARE'], ['ambalare-ax-526-9278','AMBALARE AX-526-9278'] ] },
  ...
  registry.helperAcl.buttonGroups = [
    ...
    { key:'manuale-instructiuni', label:'Submeniul MANUALE / INSTRUCȚIUNI', items: flattenIndexButtons([indexItemByKey('group-manuale-instructiuni')], []).filter(function(entry){ return entry[0] !== 'group-manuale-instructiuni'; }) },
  ...
  registry.getManagedPageEntries = function(){
    ...
    [{ page_key:'group-resurse-umane', page_name:'Grup / Resurse umane' }, { page_key:'group-resurse-umane-pontaje', page_name:'Grup / Resurse umane / Pontaje' }, { page_key:'group-resurse-umane-evaluari', page_name:'Grup / Resurse umane / Evaluări' }, { page_key:'group-manuale-instructiuni', page_name:'Grup / Manuale și instrucțiuni' }, { page_key:'group-administrator', page_name:'Grup / Administrator' }].forEach(function(item){ out.push(item); });
    return out;
  };

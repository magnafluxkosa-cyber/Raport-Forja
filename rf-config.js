/* === K.A.D LOCAL STORAGE QUOTA GUARD ===
   Păstrează spațiu liber pentru tokenul Supabase și curăță automat cache-urile locale mari/vechi.
   Nu modifică datele din Supabase; acționează doar în browserul utilizatorului. */
(function(window){
  'use strict';
  if(!window || window.__KAD_STORAGE_GUARD_INSTALLED__) return;
  window.__KAD_STORAGE_GUARD_INSTALLED__ = true;

  var MAX_TOTAL_BYTES = 3.6 * 1024 * 1024;
  var TARGET_BYTES = 2.4 * 1024 * 1024;
  var LARGE_ENTRY_BYTES = 512 * 1024;
  var CLEANUP_COOLDOWN_MS = 3000;
  var lastCleanupAt = 0;

  var PROTECTED_EXACT = Object.create(null);
  [
    'erp_forja_config','ERP_SUPABASE_URL','ERP_SUPABASE_ANON_KEY',
    'rf_theme_palette','rf_index_palette_v1','kad_pwa_install_dismissed_until_v1',
    'rf_uid','rf_email','rf_role','rf_login_ts','rf_idle_last_activity_at',
    'rf_user_id','rf_user_email','rf_user_role','rf_login_at','rf_cached_role','rf_display_id',
    'rf_force_stay_on_login','rf_acl_denied_message','rf_login_error','rf_hidden_index_buttons',
    'KAD_ENABLE_INDEX_VISIBILITY_REALTIME','KAD_ENABLE_LIVE_PRESENCE'
  ].forEach(function(key){ PROTECTED_EXACT[key] = true; });

  var DISPOSABLE_PATTERNS = [
    /cache/i,
    /autosave/i,
    /_index_/i,
    /:index:/i,
    ...
    /source_snapshot/i,
    /snapshot/i,
    /^kad_capabilitate_proces_forjare_pdf_cache/i,
    /^kad-prelucrari-grid:/i,
    /^kad-planificare-prelucrari-cache/i,
    /^rf_inventar_otel_master_cache/i,
    /^LISTA_VANZARI_v/i,
    /^rf:magnaflux:cache/i,
    /^rf:magnaflux:calendar:cache/i,
    /^rf:intrari-otel:cache/i,
    /^rf-.*-cache/i,
    /^kad_.*_cache/i,
    /^kad\..*\.preview\./i
  ];
  ...
  function bytesOf(key, value){ return (String(key || '').length + String(value || '').length) * 2; }
  function isQuotaError(err){
    var name = String(err && err.name || '');
    var msg = String(err && err.message || '');
    return err && (err.code === 22 || err.code === 1014 || /quota|exceed|exceeded|storage/i.test(name + ' ' + msg));
  }
  function isProtectedKey(key){
    var k = String(key || '');
    if(!k) return true;
    if(PROTECTED_EXACT[k]) return true;
    if(/^sb-[a-z0-9_-]+-auth-token$/i.test(k)) return true;
    if(/^sb-.*auth.*token/i.test(k)) return true;
    if(/^supabase\.auth\./i.test(k)) return true;
    if(/^rf_(uid|email|role|login|user|cached|display|idle|acl|hidden)/i.test(k) && !/cache|autosave|index/i.test(k)) return true;
    if(/^kad:forja-ctc:unlocked-/i.test(k)) return true;
    if(/^kad:operator-debitare:unlock/i.test(k)) return true;
    return false;
  }
  function isDisposableKey(key){
    var k = String(key || '');
    for(var i=0;i<DISPOSABLE_PATTERNS.length;i++){
      if(DISPOSABLE_PATTERNS[i].test(k)) return true;
    }
    return false;
  }
  function getStore(){
  ...
  function collectEntries(store){
    var entries = [];
    var total = 0;
    if(!store) return { entries: entries, total: 0 };
    try {
      for(var i=0;i<store.length;i++){
        var key = store.key(i);
        var value = store.getItem(key);
        var size = bytesOf(key, value);
        total += size;
        entries.push({ key: key, value: value, bytes: size, disposable: isDisposableKey(key), protected: isProtectedKey(key) });
      }
    } catch(_) {}
  ...
  function cleanupStorage(options){
    options = options || {};
    var store = getStore();
    if(!store) return { ok:false, removed:0, before:0, after:0 };

    var aggressive = options.aggressive === true;
    var force = options.force === true;
    if(!force && !aggressive && now() - lastCleanupAt < CLEANUP_COOLDOWN_MS){
      var current = collectEntries(store);
      return { ok:true, skipped:true, removed:0, before:current.total, after:current.total };
    }
    lastCleanupAt = now();

    var data = collectEntries(store);
    var before = data.total;
    var target = Number(options.targetBytes || (aggressive ? TARGET_BYTES : TARGET_BYTES));
    if(!aggressive && before <= MAX_TOTAL_BYTES){
      return { ok:true, removed:0, before:before, after:before };
    }

    var candidates = data.entries.filter(function(item){
      if(!item || item.protected) return false;
      if(item.disposable) return true;
      if(item.bytes >= LARGE_ENTRY_BYTES) return true;
      return aggressive === true;
    });

    candidates.sort(function(a,b){
      var scoreA = (a.disposable ? 1000000000 : 0) + a.bytes;
      var scoreB = (b.disposable ? 1000000000 : 0) + b.bytes;
    ...
    for(var i=0;i<candidates.length;i++){
      if(total <= target && removed > 0) break;
      if(removeKey(store, candidates[i].key)){
    ...
    try {
      if(removed && window.console && console.info) {
        console.info('K.A.D storage guard: cache local curățat', { removed: removed, beforeKB: Math.round(before/1024), afterKB: Math.round(Math.max(0,total)/1024), reason: options.reason || '' });
      }
    } catch(_) {}
    return { ok:true, removed:removed, before:before, after:Math.max(0,total) };
  }
  ...
  function usage(){
    ...
    var data = collectEntries(store);
    return {
      totalKB: Math.round(data.total / 1024),
      entries: data.entries.map(function(item){ return { key:item.key, KB:Math.round(item.bytes/1024), disposable:item.disposable, protected:item.protected }; }).sort(function(a,b){ return b.KB - a.KB; })
    };
  }

  function installSetItemPatch(){
    try {
      if(!window.Storage || !window.Storage.prototype || window.Storage.prototype.__kadQuotaGuardPatched) return;
      ...
      proto.setItem = function(key, value){
        try {
          ...
          if(isQuotaError(err)){
            cleanupStorage({ aggressive:true, force:true, reason:'quota:' + String(key || '') });
            try { return originalSetItem.call(this, key, value); } catch(err2) { throw err2; }
          }
          throw err;
        }
      };
      proto.__kadQuotaGuardPatched = true;
    } catch(_) {}
  }

  window.KADStorageGuard = window.KADStorageGuard || {};
  window.KADStorageGuard.cleanup = cleanupStorage;
  window.KADStorageGuard.usage = usage;
  window.KADStorageGuard.isProtectedKey = isProtectedKey;
  window.KADStorageGuard.isDisposableKey = isDisposableKey;

  installSetItemPatch();
  cleanupStorage({ reason:'startup' });
})(window);
/* === END K.A.D LOCAL STORAGE QUOTA GUARD === */
...
(function (window) {
  ...
  var PAGE_LIST = [
    ...
    { page_key: 'access-gate', page_name: 'Access Gate / PIN Portal' },
    ...
    { page_key: 'group-mentenanta', page_name: 'Grup / Mentenanță' },
    { page_key: 'group-manuale-instructiuni', page_name: 'Grup / Manuale și instrucțiuni' },
    { page_key: 'group-administrator', page_name: 'Grup / Administrator' },
    ...
    { page_key: 'manual-cm-315t', page_name: 'Manual CM 3.15 T' },
    { page_key: 'manual-cm-25t', page_name: 'Manual CM 2.5 T' },
    { page_key: 'magnaflux', page_name: 'Magnaflux' },
    { page_key: 'capabilitate-proces-forjare', page_name: 'Capabilitate Proces Forjare' },
    { page_key: 'capabilitate-proces-prelucrari', page_name: 'Capabilitate Proces Prelucrări' },
    ...
    { page_key: 'tratament-termic-fisa-autocontrol-zale', page_name: 'Tratament Termic - Fișă autocontrol' },
    ...
    { page_key: 'pontaj-forja', page_name: 'Pontaj Forja' },
    { page_key: 'pontaj-mecanici', page_name: 'Pontaj Mecanici' },
...
var PAGE_CONTROL_OVERRIDES = Object.freeze({
  'index': Object.freeze([
    ...
    Object.freeze({ control_key:'nav.group-manuale-instructiuni', control_label:'Buton MANUALE / INSTRUCȚIUNI', control_type:'action' }),
    Object.freeze({ control_key:'nav.manual-cm-315t', control_label:'Buton Manual CM 3.15 T', control_type:'action' }),
    Object.freeze({ control_key:'nav.manual-cm-25t', control_label:'Buton Manual CM 2.5 T', control_type:'action' }),
    ...
    Object.freeze({ control_key:'nav.tratament-termic-fisa-autocontrol-zale', control_label:'Buton T.T FIȘĂ AUTOCONTROL', control_type:'action' }),
    ...
    Object.freeze({ control_key:'nav.pontaj-forja', control_label:'Buton PONTAJ FORJA', control_type:'action' }),
    Object.freeze({ control_key:'nav.pontaj-mecanici', control_label:'Buton PONTAJ MECANICI', control_type:'action' }),
  ...
  'arhiva-documente': Object.freeze([
    ...
    Object.freeze({ control_key:'archive-documents.form', control_label:'Formular Arhivă Documente', control_type:'field' }),
    Object.freeze({ control_key:'archive-documents.filter', control_label:'Filtrare Arhivă Documente', control_type:'field' }),
    Object.freeze({ control_key:'archive-documents.save', control_label:'Salvare document arhivă', control_type:'action' }),
    Object.freeze({ control_key:'archive-documents.upload', control_label:'Încărcare fișier document', control_type:'action' }),
    Object.freeze({ control_key:'archive-documents.export', control_label:'Export Arhivă Documente', control_type:'action' }),
    Object.freeze({ control_key:'field.document-type', control_label:'Tip document', control_type:'field' }),
    Object.freeze({ control_key:'field.category', control_label:'Categorie document', control_type:'field' }),
    Object.freeze({ control_key:'field.retention', control_label:'Retenție document', control_type:'field' }),
    Object.freeze({ control_key:'field.document-date', control_label:'Data documentului', control_type:'field' }),
    Object.freeze({ control_key:'field.part-no', control_label:'Reper', control_type:'field' }),
    Object.freeze({ control_key:'field.cat-code', control_label:'Cod CAT', control_type:'field' }),
    Object.freeze({ control_key:'field.lot-no', control_label:'Lot / Sarjă', control_type:'field' }),
    Object.freeze({ control_key:'field.revision', control_label:'Revizie', control_type:'field' }),
    Object.freeze({ control_key:'field.source', control_label:'Sursă document', control_type:'field' }),
    Object.freeze({ control_key:'field.status', control_label:'Status document', control_type:'field' }),
    Object.freeze({ control_key:'field.title', control_label:'Denumire document', control_type:'field' }),
    Object.freeze({ control_key:'field.notes', control_label:'Observații document', control_type:'field' })
  ]),
...
(function () {
  ...
  var OPERATOR_SHEET_PAGES_NO_DASHBOARD = {
  ...
  function isOperatorRole(role) {
    ...
    return role === 'operator' ||
      role.indexOf('operator-') === 0 ||
      role.indexOf('-operator') !== -1 ||
      role.indexOf('operator') !== -1;
  }

  function isOperatorSheetPage(pageKey) {
    pageKey = normalize(String(pageKey || '')).replace(/\s+/g, '-');
    return !!OPERATOR_SHEET_PAGES_NO_DASHBOARD[pageKey];
  }

  function isOperatorUiContext() {
    try {
      var txt = normalize((document.getElementById('roleChip') || {}).textContent || '');
      return txt.indexOf('operator') !== -1;
    } catch (_) {
      return false;
    }
  }

  function isOperatorEmailContext() {
    try {
      var email = normalize(readStorage('rf_user_email') || readStorage('user_email') || readStorage('email'));
      return email === 'op@tratament.local' || email.indexOf('@tratament.local') !== -1;
    } catch (_) {
      return false;
    }
  }
  ...
  async function refreshDashboardBackButton() {
    ...
    try {
      ...
      var operator = isOperatorRole(role) || isOperatorSheetPage(pageKey) || isOperatorUiContext() || isOperatorEmailContext();
...
(function(window, document){
  ...
  var PWA_ASSET_VERSION = '20260602-storage-guard-v1';
  var MANIFEST_HREF = '/site.webmanifest?v=' + PWA_ASSET_VERSION;
  var SERVICE_WORKER_HREF = '/service-worker.js?v=' + PWA_ASSET_VERSION;
  ...
  function registerServiceWorker(){
    try {
      ...
      navigator.serviceWorker.register(SERVICE_WORKER_HREF, { scope: '/' }).then(function(registration){
        try { if(registration && typeof registration.update === 'function') registration.update(); } catch (_) {}
        try {
          var worker = registration && (registration.active || registration.waiting || registration.installing);
          if(worker && typeof worker.postMessage === 'function') worker.postMessage({ type:'KAD_CLEAR_SW_CACHES' });
        } catch (_) {}
      }).catch(function(err){

(() => {
      ...
      const state = {
        ...
        ttRuleMap: new Map(),
        problemRows: [],
        problemAggregateMap: new Map(),
        formMinutesAutoLinked: false,
        formProblemLinkKey: ''
      };
      ...
      function makeProblemLinkKey(source) {
        const row = source || {};
        const data = toStr(row.data);
        const schimbul = toStr(row.schimbul);
        const operator = normalizeText(row.operator);
        if (!data || !schimbul || !operator) return '';
        return [data, schimbul, operator].join('|');
      }

      function categoryToMinuteField(value) {
        const category = normalizeText(value);
        if (!category) return '';
        if (category === 'MENTENANTA' || category === 'MENTENANȚĂ') return 'mentenanta';
        if (category === 'INCALZIRE' || category === 'ÎNCĂLZIRE') return 'incalzire';
        if (category === 'GOLIRE') return 'golire';
        if (category === 'OPRIRI NEPLANIFICATE' || category === 'ALTA PROBLEMA' || category === 'ALTĂ PROBLEMĂ') return 'opriri_neplanificate';
        return '';
      }

      function rebuildProblemAggregateMap() {
        const map = new Map();
        (state.problemRows || []).forEach((rawRow) => {
          const row = rawRow || {};
          const key = makeProblemLinkKey(row);
          if (!key) return;
          const field = categoryToMinuteField(row.categorie);
          if (!field) return;
          const minutes = toNumber(row.minute);
          if (minutes <= 0) return;
          const bucket = map.get(key) || {
            opriri_neplanificate: 0,
            mentenanta: 0,
            incalzire: 0,
            golire: 0,
            totalMinute: 0,
          ...
          bucket[field] = round2(bucket[field] + minutes);
          bucket.totalMinute = round2(bucket.totalMinute + minutes);
          bucket.count += 1;
          map.set(key, bucket);
        });
        state.problemAggregateMap = map;
      }

      function getProblemAggregate(source) {
        const key = makeProblemLinkKey(source);
        if (!key) return null;
        const bucket = state.problemAggregateMap.get(key);
        if (!bucket) return null;
        return Object.assign({ key }, bucket);
      }

      function applyProblemAggregateToRow(source) {
        const row = normalizeRow(source || {});
        const aggregate = getProblemAggregate(row);
        if (!aggregate || aggregate.count <= 0) return row;
        const linkedRow = Object.assign({}, row, {
          opriri_neplanificate: round2(aggregate.opriri_neplanificate),
          mentenanta: round2(aggregate.mentenanta),
          incalzire: round2(aggregate.incalzire),
          golire: round2(aggregate.golire)
        });
        return normalizeRow(linkedRow);
      }

      function syncStoredRowsWithProblemLinks() {
        state.rows = (state.rows || []).map((row) => applyProblemAggregateToRow(row));
      }

      function getIssueCoverageDetails(source) {
        const row = source || {};
        const opriri = toNumber(row.opriri_neplanificate);
        const mentenanta = toNumber(row.mentenanta);
        const incalzire = toNumber(row.incalzire);
        const golire = toNumber(row.golire);
        const totalMinute = opriri + mentenanta + incalzire + golire;
        return {
          opriri,
          mentenanta,
          incalzire,
          golire,
          totalMinute: round2(totalMinute),
          fullShiftCovered: totalMinute >= SHIFT_MINUTES
        };
      }
      ...
      function readForm() {
        syncYearMonthFromDate();
        const linkedAggregate = getProblemAggregate({
          data: els.fldData.value,
          schimbul: els.fldSchimbul.value,
          operator: els.fldOperator.value
        });
        const baseMinutes = linkedAggregate && linkedAggregate.count > 0
          ? {
              opriri_neplanificate: linkedAggregate.opriri_neplanificate,
              mentenanta: linkedAggregate.mentenanta,
              incalzire: linkedAggregate.incalzire,
              golire: linkedAggregate.golire
            }
          : {
        ...
        const coveragePreview = getIssueCoverageDetails(baseMinutes);
        ...
        const base = {
          ...
          opriri_neplanificate: baseMinutes.opriri_neplanificate,
          mentenanta: baseMinutes.mentenanta,
          incalzire: baseMinutes.incalzire,
          golire: baseMinutes.golire
        };
        return applyProblemAggregateToRow(base);
      }
      ...
      function syncMinuteInputsFromProblemLink(options = {}) {
        const preserveWhenNoLink = options.preserveWhenNoLink !== false;
        const aggregate = getProblemAggregate({
          data: els.fldData.value,
          schimbul: els.fldSchimbul.value,
          operator: els.fldOperator.value
        });
        const minuteInputs = [els.fldOpriri, els.fldMentenanta, els.fldIncalzire, els.fldGolire];

        if (aggregate && aggregate.count > 0) {
          els.fldOpriri.value = aggregate.opriri_neplanificate ? String(aggregate.opriri_neplanificate) : '';
          els.fldMentenanta.value = aggregate.mentenanta ? String(aggregate.mentenanta) : '';
          els.fldIncalzire.value = aggregate.incalzire ? String(aggregate.incalzire) : '';
          els.fldGolire.value = aggregate.golire ? String(aggregate.golire) : '';
          minuteInputs.forEach((el) => { el.disabled = true; });
          state.formMinutesAutoLinked = true;
          state.formProblemLinkKey = aggregate.key;
          return aggregate;
        }

        if (state.formMinutesAutoLinked && !preserveWhenNoLink) {
          els.fldOpriri.value = '';
          els.fldMentenanta.value = '';
          els.fldIncalzire.value = '';
          els.fldGolire.value = '';
        }
        if (canEdit()) {
          minuteInputs.forEach((el) => { el.disabled = false; });
        }
        state.formMinutesAutoLinked = false;
        state.formProblemLinkKey = '';
        return null;
      }

      function fillForm(row) {
        const source = applyProblemAggregateToRow(row || {});
        ...
        syncMinuteInputsFromProblemLink({ preserveWhenNoLink: true });
        updateProductionFieldsState();
      ...
      function clearForm() {
        ...
        fillForm({});
        setDefaults();
        syncMinuteInputsFromProblemLink({ preserveWhenNoLink: false });
        updateProductionFieldsState();
        applyComputedOreToForm();
        updateSelectionUi();
        renderTable();
        requestAnimationFrame(() => {
          try {
            els.fldData.focus();
            if (typeof els.fldData.showPicker === 'function') {
              // nu forțăm calendarul, doar lăsăm focus pe câmp
            }
          } catch (_) {}
        });
      }
      ...
      function updateProductionFieldsState() {
        const coverage = getIssueCoverageDetails({
          opriri_neplanificate: els.fldOpriri.value,
          mentenanta: els.fldMentenanta.value,
          incalzire: els.fldIncalzire.value,
          golire: els.fldGolire.value
        });
        const disableProduction = coverage.fullShiftCovered;
        [els.fldReper, els.fldSarja, els.fldCantitate].forEach((el) => {
          el.disabled = disableProduction || !canEdit();
        });
        if (disableProduction) {
          els.fldReper.value = '';
          els.fldSarja.value = '';
          els.fldCantitate.value = '';
        }
      }
      ...
      function updateFormNote() {
        ...
        const calc = calculateOreDetails({
          ...
          opriri_neplanificate: linkedAggregate ? linkedAggregate.opriri_neplanificate : els.fldOpriri.value,
          mentenanta: linkedAggregate ? linkedAggregate.mentenanta : els.fldMentenanta.value,
          incalzire: linkedAggregate ? linkedAggregate.incalzire : els.fldIncalzire.value,
          golire: linkedAggregate ? linkedAggregate.golire : els.fldGolire.value
        });
        const modeText = state.selectedId ? 'Mod editare: actualizare rând selectat' : 'Mod editare: rând nou';
        const coverage = calc.coverage || getIssueCoverageDetails({
          opriri_neplanificate: linkedAggregate ? linkedAggregate.opriri_neplanificate : els.fldOpriri.value,
          mentenanta: linkedAggregate ? linkedAggregate.mentenanta : els.fldMentenanta.value,
          incalzire: linkedAggregate ? linkedAggregate.incalzire : els.fldIncalzire.value,
          golire: linkedAggregate ? linkedAggregate.golire : els.fldGolire.value
        });
        const linkedText = linkedAggregate && linkedAggregate.count > 0
          ? ' • Minute preluate automat din Probleme T.T (' + formatNumber(linkedAggregate.count, 0) + ' înregistrări)'
          : '';

        if (coverage.fullShiftCovered && toNumber(els.fldCantitate.value) <= 0 && !toStr(els.fldReper.value) && !toStr(els.fldSarja.value)) {
          els.formNote.textContent = modeText + linkedText + ' • Schimb acoperit integral de opriri/mentenanță (' + formatNumber(coverage.totalMinute, 0) + ' min) — poți salva fără reper, sarjă și cantitate; câmpurile de producție se blochează automat; mentenanța nu intră în orele consumate.';
          return;
        }

        if (!toStr(els.fldReper.value) || toNumber(els.fldCantitate.value) <= 0) {
          els.formNote.textContent = modeText + linkedText + ' • Orele se calculează automat după reper + cantitate + opriri + încălzire + golire';
          return;
        }

        if (!calc.rule) {
          els.formNote.textContent = modeText + linkedText + ' • Reperul nu are regulă de tratament termic în excelul de bază, deci orele rămân 0';
          return;
        }

        els.formNote.textContent = modeText + linkedText + ' • Ore auto: ' + formatNumber(calc.ore, 2) +
      ...
      function renderTable() {
        const rows = sortRows(state.rows.map((row) => applyProblemAggregateToRow(row)));
        if (!rows.length) {
      ...
      function setReadonlyMode(readonly) {
        ...
        [els.fldOpriri, els.fldMentenanta, els.fldIncalzire, els.fldGolire].forEach((el) => {
          el.disabled = readonly || state.formMinutesAutoLinked;
        });
        els.fldOre.disabled = true;
      ...
      async function loadProblemRowsFromCloudOrLocal() {
        let rows = [];
        try {
          const raw = localStorage.getItem(PROBLEMS_LOCAL_KEY);
          if (raw) {
        ...
        if (sb && state.auth && state.auth.user) {
          try {
            const { data, error } = await sb.from('rf_documents').select('doc_key,content,updated_at').eq('doc_key', PROBLEMS_DOC_KEY).maybeSingle();
            if (error) throw error;
            if (data && data.content && Array.isArray(data.content.rows)) {
              rows = data.content.rows.map((row) => row || {});
              try {
                localStorage.setItem(PROBLEMS_LOCAL_KEY, JSON.stringify({ rows, updated_at: data.updated_at || nowIso() }));
              } catch (_) {}
            }
          } catch (error) {
            console.error('Nu am putut încărca legătura cu Probleme T.T:', error);
          }
        }

        state.problemRows = rows;
        rebuildProblemAggregateMap();
      }
      ...
      async function loadFromCloudOrLocal() {
        ...
        syncStoredRowsWithProblemLinks();
      ...
      window.__ttSaveRow = function (event) {
        return upsertRow(event);
      };

      window.__ttNewRow = function (event) {
        if (event) {
          if (typeof event.preventDefault === 'function') event.preventDefault();
          if (typeof event.stopPropagation === 'function') event.stopPropagation();
          if (typeof event.stopImmediatePropagation === 'function') event.stopImmediatePropagation();
      ...
      function replaceButtonWithoutListeners(buttonEl) {
        if (!buttonEl || !buttonEl.parentNode) return buttonEl;
        const clone = buttonEl.cloneNode(true);
        buttonEl.parentNode.replaceChild(clone, buttonEl);
        return clone;
      }

      function bindActions() {
        els.btnSaveRow = replaceButtonWithoutListeners(els.btnSaveRow);
        els.btnNew = replaceButtonWithoutListeners(els.btnNew);
        els.btnDelete = replaceButtonWithoutListeners(els.btnDelete);
        els.btnReload = replaceButtonWithoutListeners(els.btnReload);
        els.btnCloudSave = replaceButtonWithoutListeners(els.btnCloudSave);

        els.btnSaveRow.onclick = window.__ttSaveRow;
        els.btnNew.onclick = window.__ttNewRow;
        ...
        [els.fldData, els.fldSchimbul, els.fldOperator].forEach((el) => {
          const handleSourceLinkChange = () => {
            syncYearMonthFromDate();
            syncMinuteInputsFromProblemLink({ preserveWhenNoLink: !!state.selectedId });
            updateProductionFieldsState();
            applyComputedOreToForm();
          };
          el.addEventListener('input', handleSourceLinkChange);
          el.addEventListener('change', handleSourceLinkChange);
        });
      ...
      async function init() {
        ...
        await loadProblemRowsFromCloudOrLocal();

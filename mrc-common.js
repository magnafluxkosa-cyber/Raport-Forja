(function(window){
  ...
  function replaceCustomerOrdersForClientPeriod(existingRows, importedRows){
    ...
    const kept = existing.filter(row => {
      ...
      if (importedSourceTypes.length && !importedSourceTypes.includes(rowSourceType)) return true;
  ...
  function importCustomerOrdersFromWorkbook(workbook, maps, options){
    if (window.MRCOrderImporters && typeof window.MRCOrderImporters.importCustomerOrdersFromWorkbook === 'function'){
      return window.MRCOrderImporters.importCustomerOrdersFromWorkbook(workbook, maps, options || {});
    }
    const rows = [];
    const opts = options || {};

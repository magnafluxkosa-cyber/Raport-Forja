(function(window, document){
  ...
  function injectSecurityStyle(){
    if(document.getElementById('kad-security-guard-v2-style')) return;
    var style = document.createElement('style');
    style.id = 'kad-security-guard-v2-style';
    style.textContent = [
  ...
  async function main(){
    ...
    await ensureRuntime();
    var sb = createClient();
    var session = await withTimeout(getSession(sb), 9000, null);
    if(!session || !session.user){ redirectToLogin(); return; }

    if(pageKey === 'index'){
      var indexStatus = await withTimeout(getAccountStatus(sb, session.user), 4500, { is_active:true, is_banned:false, note:'' });
      if(indexStatus && (indexStatus.is_banned === true || indexStatus.is_active === false)){
        try { await sb.auth.signOut(); } catch(_) {}
        renderDenied('Acces restricționat', indexStatus.note || 'Cont blocat.');
        return;
      }
      window.__KAD_SECURITY_ACCESS__ = {
        ...
        accountStatus:indexStatus,
        source:'authenticated index fast-path'
      };
      markAllowed();
      return;
    }
  ...
  window.KADSecurityGuard.version = 'v2';
  ...
  main().catch(function(err){
    try { console.error('KAD security guard v2', err); } catch(_) {}
